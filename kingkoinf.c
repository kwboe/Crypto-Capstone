 ...
 * If a user’s home directory isn’t available, a fallback global keys directory
 * (GLOBAL_KEYS_DIR) is used.
 *
 * Compile with:
 *   gcc -Wall -Wextra -O2 -o kingkoinf kingkoinf.c net_sync.c -lpam -lpam_misc -lcrypto -loqs -lpthread
 *
...
#include <openssl/evp.h>  // SHA3-512 via OpenSSL EVP interface
#include <security/pam_appl.h>   // PAM authentication
#include <oqs/oqs.h>      // Dilithium2
#include <pthread.h>
#include <arpa/inet.h>
#include "net_sync.h"     // Must supply required networking functions
...
void generate_global_keys_for_user(const char *username) {
  struct stat st;
  if (stat(GLOBAL_KEYS_DIR, &st) == -1) {
    if (mkdir(GLOBAL_KEYS_DIR, 0700) != 0) {
      perror("mkdir GLOBAL_KEYS_DIR");
      exit(1);
    }
  }
  char privkey[256], pubkey[256];
  snprintf(privkey, sizeof(privkey), "%s/%s_private.key", GLOBAL_KEYS_DIR, username);
  snprintf(pubkey, sizeof(pubkey), "%s/%s_public.key", GLOBAL_KEYS_DIR, username);
  if (access(privkey, F_OK) != -1 && access(pubkey, F_OK) != -1) {
    printf("Global keys already exist for '%s'.\n", username);
    return;
  }
  OQS_SIG *sig = OQS_SIG_new("Dilithium2");
  if (!sig) { printf("Dilithium2 init fail in global keys generation.\n"); exit(1); }
  unsigned char *pub = malloc(sig->length_public_key);
  unsigned char *priv = malloc(sig->length_secret_key);
  if (!pub || !priv) { perror("malloc"); exit(1); }
  if (OQS_SIG_keypair(sig, pub, priv) != OQS_SUCCESS) {
    printf("Failed global keypair for '%s'.\n", username);
    exit(1);
  }
  FILE *fp = fopen(privkey, "wb");
  if (!fp) { printf("Failed to open global private key '%s'.\n", privkey); exit(1); }
  fwrite(priv, 1, sig->length_secret_key, fp);
  fclose(fp);
  chmod(privkey, 0600);
  fp = fopen(pubkey, "wb");
  if (!fp) { printf("Failed to open global public key '%s'.\n", pubkey); exit(1); }
  fwrite(pub, 1, sig->length_public_key, fp);
  fclose(fp);
  chmod(pubkey, 0644);
  OQS_SIG_free(sig);
  free(pub);
  free(priv);
  printf("Generated global keys for '%s'.\n", username);
}
...
void load_user_public_key(User *user) {
  ...
  char pkey[256];
  if (pwd)
    snprintf(pkey, sizeof(pkey), "%s/.blockchain_keys/%s_public.key", pwd->pw_dir, user->username);
  else
...
void finalize_block(void) {
  ...
  char msg[512];
  snprintf(msg, sizeof(msg), "NEW_BLOCK:%d:", pending_block->index);
  for (unsigned int i = 0; i < pending_block->hash_len; i++) {
    char hex[3];
    snprintf(hex, sizeof(hex), "%02x", pending_block->hash[i]);
    strncat(msg, hex, sizeof(msg) - strlen(msg) - 1);
  }
...
void sign_block(Block *block, const char *validator_username) {
  ...
  if (vidx == -1) { printf("User '%s' is not a validator.\n", validator_username); return; }
  struct passwd *pwd = getpwnam(validator_username);
  char privkey[256];
  if (pwd)
    snprintf(privkey, sizeof(privkey), "%s/.blockchain_keys/%s_private.key", pwd->pw_dir, validator_username);
  else {
    printf("getpwnam('%s') failed; falling back to GLOBAL_KEYS_DIR.\n", validator_username);
    snprintf(privkey, sizeof(privkey), "%s/%s_private.key", GLOBAL_KEYS_DIR, validator_username);
    if (access(privkey, F_OK) == -1) {
      printf("Global private key not found for '%s'; generating...\n", validator_username);
      generate_global_keys_for_user(validator_username);
    }
  }
  FILE *fp = fopen(privkey, "rb");
  if (!fp) { printf("Failed to open private key for '%s' (tried '%s').\n", validator_username, privkey); return; }
  OQS_SIG *sig = OQS_SIG_new("Dilithium2");
  ...
  if (fread(private_key, 1, sig->length_secret_key, fp) != sig->length_secret_key) {
    ...
    fclose(fp); free(private_key); OQS_SIG_free(sig); return;
  }
  fclose(fp);
  ...
  if (OQS_SIG_sign(sig, block->signatures[vidx], &block->signature_lens[vidx],
    ...
    free(private_key); OQS_SIG_free(sig); return;
  }
  printf("Block signed by '%s'.\n", validator_username);
  free(private_key); OQS_SIG_free(sig);
}
...
int verify_block_signatures(Block *block) {
  ...
  for (int i = 0; i < block->validator_count; i++) {
    ...
    if (!u) { printf("Validator '%s' not found.\n", block->validators[i]); all_valid = 0; continue; }
    if (u->public_key_len == 0) { printf("Public key for '%s' is missing.\n", u->username); all_valid = 0; continue; }
    int r = (OQS_SIG_verify(sig, block_data, off,
                              block->signatures[i], block->signature_lens[i],
                              u->public_key) == OQS_SUCCESS);
    if (!r) { printf("Block %d signature verification failed for '%s'.\n", block->index, u->username); all_valid = 0; }
  }
  OQS_SIG_free(sig);
  return all_valid;
}
...
void sign_pending_block(const char *validator_username) {
  ...
  if (vidx == -1) { printf("User '%s' is not a validator for the pending block.\n", validator_username); return; }
  ...
  if (all_signed) { printf("All validators have signed. Finalizing block...\n"); finalize_block(); }
...
void user_login(void) {
  ...
  if (retval != PAM_SUCCESS) { printf("pam_start fail: %s\n", pam_strerror(pamh, retval)); return; }
  retval = pam_authenticate(pamh, 0);
  if (retval != PAM_SUCCESS) { printf("Authentication failed: %s\n", pam_strerror(pamh, retval)); pam_end(pamh, retval); return; }
  retval = pam_acct_mgmt(pamh, 0);
  if (retval != PAM_SUCCESS) { printf("Account management failed: %s\n", pam_strerror(pamh, retval)); pam_end(pamh, retval); return; }
