...
// Function to remove newline characters from the data
void remove_newlines(unsigned char *data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        if (data[i] == '\n' || data[i] == '\r') {
            data[i] = ' ';  // Replace newline or carriage return with a space or null character
        }
    }
}
...
int main() {
    ...
    // Bob generates his public and secret keys
    crypto_kem_keypair(pk, sk);
    printf("Bob: Public key generated.\n");

    // Null terminate the public key and secret key, remove newline characters
    pk[CRYPTO_PUBLICKEYBYTES - 1] = '\0';
    sk[CRYPTO_SECRETKEYBYTES - 1] = '\0';

    remove_newlines(pk, CRYPTO_PUBLICKEYBYTES);
    remove_newlines(sk, CRYPTO_SECRETKEYBYTES);
    ...
    server_addr.sin_addr.s_addr = inet_addr("10.233.105.182");  // Assuming Alice is on the same machine

    // Connect to Alice (server)
    ...
    // Null terminate the ciphertext and remove newlines
    ciphertext[CRYPTO_CIPHERTEXTBYTES - 1] = '\0';
    remove_newlines(ciphertext, CRYPTO_CIPHERTEXTBYTES);
