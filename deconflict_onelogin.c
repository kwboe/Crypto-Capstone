...
int key_agree(unsigned char *pk, unsigned char *sk, unsigned char *servpk, 
    ...
    print_hex("Our pk", pk, CRYPTO_PUBLICKEYBYTES); [debug statement]
...
int aes_ctr_decrypt(const unsigned char *key, const unsigned char *ciphertext, size_t ciphertext_len,
    ...
    if (!EVP_DecryptInit_ex(ctx, EVP_aes_256_ctr(), NULL, key, iv)) {
        perror("Error, cannot initiate decryption");
        EVP_CIPHER_CTX_free(ctx);
        return -1;
    }
    ...
    if (!EVP_DecryptFinal_ex(ctx, plaintext + plaintext_len, &len)) {
        perror("Error, during final decryption step");
        EVP_CIPHER_CTX_free(ctx);
        return -1;
    }
    plaintext_len += len;
...
void send_all(int sockfd, const void *buffer, size_t length) {
    size_t sent = 0;
    while (sent < length) {
        ssize_t bytes = send(sockfd, (const char*)buffer + sent, length - sent, 0);
        if (bytes < 0) {
            perror("send error");
            exit(1);
        }
...
void recv_all(int sockfd, void *buffer, size_t length) {
    size_t received = 0;
    while (received < length) {
        ssize_t bytes = recv(sockfd, (char*)buffer + received, length - received, 0);
        if (bytes <= 0) {
            perror("recv error");
            exit(1);
        }
